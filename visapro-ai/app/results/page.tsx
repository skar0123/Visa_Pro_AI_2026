...
import type { RoadmapItem, RFEPrediction, ApprovalSimulation, SectionResult, VisaProbabilities } from "@/lib/ai";
...
interface StoredResult {
  // Always returned (free + paid)
  overall_score: number;
  visa_probabilities: VisaProbabilities;
  isPremiumLocked?: boolean;
  applicantName: string;
  // Paid tier only (may be absent for free users)
  sections?: {
...
function GateOverlay({
  ...
  label: string;
  onUpgrade: (plan: "PRO" | "PREMIUM", label: string) => void;
}) {
  const color = requiredPlan === "PREMIUM" ? "#a78bfa" : "#00d4ff";
  return (
    <div
      style={{
        position: "absolute",
    ...
    >
      ...
      <p style={{ fontSize: 13, color: "rgba(148,163,184,0.8)", lineHeight: 1.6, maxWidth: 260 }}>
        {label} requires the <strong style={{ color: "#e2e8f0" }}>{requiredPlan}</strong> plan.
      </p>
      <button
        onClick={() => onUpgrade(requiredPlan, label)}
        style={{
          padding: "9px 20px",
          borderRadius: 10,
...
export default function ResultsPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; requiredPlan: "PRO" | "PREMIUM"; label: string }>({
    open: false,
    requiredPlan: "PRO",
    label: "",
  });

  function openUpgrade(requiredPlan: "PRO" | "PREMIUM", label: string) {
    setModal({ open: true, requiredPlan, label });
  }
  ...
  const {
    ...
    applicantName, rfe_predictions, approval_simulation, isPremiumLocked,
  } = result;
  const isPaid = isPremiumLocked !== true;
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#08090f", color: "#ffffff" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "88px 24px 80px" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            {isPaid ? (
              ...
              <button
                onClick={() => openUpgrade("PREMIUM", "AI Interview Simulator")}
                style={{
                  fontSize: 12, fontWeight: 600, padding: "6px 14px", borderRadius: 8,
                  color: "rgba(139,92,246,0.5)", background: "rgba(139,92,246,0.04)", border: "1px solid rgba(139,92,246,0.12)",
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} style={{ marginBottom: 20, position: "relative", borderRadius: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
            ...
            {!isPaid && (
              <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 999, background: "rgba(0,212,255,0.08)", border: "1px solid rgba(0,212,255,0.2)", color: "#00d4ff" }}>PRO</span>
            )}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.32 }}
          style={{ borderRadius: 16, border: "1px solid rgba(255,255,255,0.08)", backgroundColor: "#0d1017", overflow: "hidden", marginBottom: 24, position: "relative" }}
        >
          {/* Tab bar */}
          ...
          {!isPaid && (
            <GateOverlay requiredPlan="PRO" label="Gap Analysis, Roadmap, RFE Predictor & Approval Simulation" onUpgrade={openUpgrade} />
          )}
        ...
        >
          ...
          {isPaid ? (
            ...
            >
              {isPaid ? "📄 Export Full PDF ↗" : "Export Report ↗"}
            </button>
          ) : (
            <button
              onClick={() => openUpgrade("PRO", "PDF Export")}
              style={{
                flex: 1,
                minWidth: 140,
                padding: "14px 20px",
        ...
        {/* Case Comparison Engine — PRO+ gated */}
        <motion.div
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: "#ffffff", margin: 0 }}>How You Compare</h2>
            <span style={{ fontSize: 11, color: "#64748b", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, padding: "2px 8px" }}>vs Approved Cases</span>
            {!isPaid && (
              <span style={{ fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 999, background: "rgba(0,212,255,0.08)", border: "1px solid rgba(0,212,255,0.2)", color: "#00d4ff" }}>PRO</span>
            )}
          </div>
          {isPaid ? (
            ...
            <div style={{ position: "relative", borderRadius: 16, overflow: "hidden", minHeight: 280 }}>
              {/* Blurred preview */}
              <div style={{ filter: "blur(6px)", pointerEvents: "none", opacity: 0.4, borderRadius: 16, border: "1px solid rgba(255,255,255,0.08)", padding: 24, background: "#0d1017" }}>
                <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
                  {["EB-1A", "O-1A", "EB-2 NIW"].map((t) => (
                    <div key={t} style={{ padding: "6px 16px", borderRadius: 8, background: "rgba(0,212,255,0.1)", border: "1px solid rgba(0,212,255,0.2)", fontSize: 12, fontWeight: 600, color: "#00d4ff" }}>{t}</div>
                  ))}
                </div>
                {["Publications", "Citations", "Awards", "Media", "Experience"].map((m) => (
                  <div key={m} style={{ marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontSize: 12, color: "#94a3b8" }}>{m}</span>
                      <span style={{ fontSize: 11, color: "#64748b" }}>You vs Benchmark</span>
                    </div>
                    <div style={{ height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3 }}>
                      <div style={{ height: "100%", width: `${30 + Math.random() * 40}%`, background: "linear-gradient(90deg, #0066ff, #00d4ff)", borderRadius: 3 }} />
                    </div>
                  </div>
                ))}
              </div>
              {/* Gate overlay */}
              <GateOverlay requiredPlan="PRO" label="Case Comparison Engine" onUpgrade={openUpgrade} />
            </div>
          )}
        </motion.div>
        ...
        <div style={{ textAlign: "center", paddingBottom: 24 }}>
          <span style={{
            ...
            padding: "4px 12px",
            borderRadius: 999,
            background: isPaid ? "rgba(0,212,255,0.08)" : "rgba(255,255,255,0.05)",
            border: isPaid ? "1px solid rgba(0,212,255,0.2)" : "1px solid rgba(255,255,255,0.08)",
            color: isPaid ? "#00d4ff" : "#64748b",
          }}>
            {isPaid ? "Paid Plan · Full access" : "Free Plan · Upgrade for full analysis"}
          </span>
        </div>
      </div>
