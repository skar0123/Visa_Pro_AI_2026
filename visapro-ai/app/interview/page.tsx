...
export default function InterviewPage() {
  ...
  useEffect(() => {
    // Check paid status via server-side session cookie
    fetch("/api/session/check")
      .then((r) => r.json())
      .then((d) => setIsPaid(d.isPaid === true))
  ...
  // Gate: paid session required. Show upgrade wall if session check loaded and not paid.
      ...
      <main style={{ minHeight: "100vh", backgroundColor: "#03050f", color: "#fff", paddingTop: 80 }}>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "80px 20px", textAlign: "center" }}>
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <div style={{ width: 64, height: 64, borderRadius: 18, background: "rgba(139,92,246,0.15)", border: "1px solid rgba(139,92,246,0.35)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, margin: "0 auto 24px" }}>
              🔒
            </div>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#a78bfa", marginBottom: 12 }}>
              Premium Feature
            </div>
            <h1 style={{ fontSize: 26, fontWeight: 700, color: "#fff", marginBottom: 12, lineHeight: 1.3 }}>
              AI Interview Simulator
            </h1>
            <p style={{ fontSize: 14, color: "rgba(100,116,139,0.8)", lineHeight: 1.7, marginBottom: 28 }}>
              Practice real USCIS-style interview questions with AI-powered attorney feedback. Available exclusively on the <strong style={{ color: "#e2e8f0" }}>Premium</strong> plan.
            </p>

            <div style={{ background: "rgba(139,92,246,0.06)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 14, padding: "20px 24px", marginBottom: 28, textAlign: "left" }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#a78bfa", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 14 }}>
                What you get with Premium
              </div>
              {[
                "AI Interview Simulator with real-time Q&A",
                "USCIS-style questions for O-1A, EB-1A, EB-2 NIW",
                "Attorney-grade answer feedback & scoring",
                "Attorney-grade petition checklist",
                "Full downloadable PDF report",
                "Advanced 12-month case strategy",
                "Unlimited evaluations",
              ].map((f) => (
                <div key={f} style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 8, fontSize: 13, color: "rgba(203,213,225,0.85)" }}>
            ...
            <div style={{ display: "flex", gap: 6, justifyContent: "center", flexWrap: "wrap", marginBottom: 28 }}>
              {["Attorney-level insights", "Used for EB-1A / O-1 success", "Deep AI evaluation engine"].map((tag) => (
                <span key={tag} style={{ fontSize: 10, fontWeight: 600, padding: "3px 10px", borderRadius: 999, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(148,163,184,0.8)" }}>
                  {tag}
                </span>
              ))}
            </div>
            ...
            <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
              <Link href="/#pricing" style={{ textDecoration: "none" }}>
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  style={{ padding: "14px 32px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #7c3aed, #a78bfa)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 24px rgba(139,92,246,0.35)" }}
                >
                  Upgrade to Premium →
                </motion.button>
              </Link>
              <Link href="/results" style={{ textDecoration: "none" }}>
                <button style={{ padding: "14px 24px", borderRadius: 12, border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "#64748b", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>
                  ← Back to Results
                </button>
              </Link>
            </div>

            <p style={{ fontSize: 12, color: "rgba(100,116,139,0.5)", marginTop: 24 }}>
              Premium: ₹8,999 / $108 per month
            </p>
          </motion.div>
        </div>
      </main>
