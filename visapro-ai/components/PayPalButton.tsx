...
export default function PayPalButton({ plan, onSuccess, onError, onCancel }: PayPalButtonProps) {
  ...
  useEffect(() => {
    ...
    console.log("[PayPal] NEXT_PUBLIC_PAYPAL_CLIENT_ID:", clientId ? `${clientId.slice(0, 10)}…` : "NOT SET");
    if (!clientId) {
      setStatus("error");
      onError("PayPal client ID not configured. Set NEXT_PUBLIC_PAYPAL_CLIENT_ID in Vercel environment variables.");
      return;
    }
