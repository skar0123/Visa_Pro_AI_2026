...
// ssr: false prevents hydration mismatch — PayPal SDK is browser-only
...
const INDIA_PLANS: Plan[] = [
  ...
  },
  {
    ...
    popular: true,
    cta: `Upgrade to Pro – ₹${PRICING.PRO.INR.toLocaleString("en-IN")}`,
    features: [
  ...
  },
  {
    ...
    popular: false,
    cta: `Get Premium – ₹${PRICING.PREMIUM.INR.toLocaleString("en-IN")}`,
    features: [
...
const INTL_PLANS: Plan[] = [
  ...
  },
  {
    ...
    glowColor: "rgba(0,212,255,0.15)",
    popular: true,
    cta: `Upgrade to Pro – $${PRICING.PRO.USD}`,
    features: [
      { text: "Full AI visa evaluation", included: true },
      { text: "Overall readiness score", included: true },
      { text: "USCIS-style gap analysis", included: true },
  ...
  },
  {
    ...
    glowColor: "rgba(139,92,246,0.15)",
    popular: false,
    cta: `Get Premium – $${PRICING.PREMIUM.USD}`,
    features: [
      { text: "Everything in Pro", included: true },
      { text: "Advanced 12-month strategy plan", included: true },
      { text: "Priority AI analysis", included: true },
...
// ── Payment config debug banner ───────────────────────────────────────────────
function PaymentDebugBanner({ tab }: { tab: "india" | "international" }) {
  const paypalClientId = process.env.NEXT_PUBLIC_PAYPAL_CLIENT_ID;
  const razorpayKeyId = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID;

  const missingKeys: string[] = [];
  if (tab === "international" && !paypalClientId) missingKeys.push("NEXT_PUBLIC_PAYPAL_CLIENT_ID");
  if (tab === "india" && !razorpayKeyId) missingKeys.push("NEXT_PUBLIC_RAZORPAY_KEY_ID");
  ...
  return (
    ...
    >
      ⚠ Missing env {missingKeys.length > 1 ? "variables" : "variable"}:{" "}
      <code style={{ fontFamily: "monospace", background: "rgba(0,0,0,0.3)", padding: "1px 6px", borderRadius: 4 }}>
        {missingKeys.join(", ")}
      </code>
      {" "}— payments are unavailable until these are set in Vercel.
    </div>
  );
}
  ...
  return (
    <section id="pricing" style={{ padding: "120px 24px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        {/* Payment config debug banner (only shows when a key is missing) */}
        <PaymentDebugBanner tab={tab} />
