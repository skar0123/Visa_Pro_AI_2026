...
export async function generateComparisonNarrative(
  ...
  const gapLines = Object.entries(gaps)
    .map(([k, v]) => `  ${k}: user=${userMetrics[k]} vs benchmark=${benchmarkMetrics[k]} (gap=${v > 0 ? "+" : ""}${v})`)
    .join("\n");

  const prompt = `You are a senior US immigration attorney specializing in extraordinary ability visas (EB-1A, O-1A, EB-2 NIW).

A candidate's profile has been compared against the median approved ${visaType} petitioner. Their overall percentile is ${percentile}/100.

Metric comparison (user vs median approved ${visaType} applicant):
${gapLines}

Write a 3–4 sentence critical, realistic assessment of how this candidate compares to approved ${visaType} applicants. Be direct: state their overall standing, name the 1–2 most important gaps, name the 1–2 strongest areas, and give one actionable priority for the next 6 months. Do not use bullet points. Write in plain prose. Do not be misleadingly encouraging — USCIS denials are common at this level.`;
